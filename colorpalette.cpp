...
void ColorPalette::setButtonColor(DoubleclickButton *button, QColor color)
{
    button->setStyleSheet(QString("background-color: %1")
                          .arg(color.name()));
}

void ColorPalette::createColorPalette()
{
    //Row1
    ...
    layout->setGeometry(QRect(QPoint(100, 200), QSize(11,16)));
...
void ColorPalette::makeConnections(DoubleclickButton *button, int index)
{
    ...
    connect(button,&DoubleclickButton::doubleClicked,[=](){
        QColor color = colorDialog.getColor(colors.at(index), this, "Pick Color");
        if(color.isValid()){
...
void ColorPalette::addColorToPalette(QString color)
{
    QColor rgbColor = (QColor)color;
